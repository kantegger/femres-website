...
By submitting a contribution, you agree that application code and technical
documentation may be distributed under the repository's MIT License. Original
editorial writing and original translations submitted to `src/content/` may be
distributed under CC BY-NC-SA 4.0. Do not submit third-party text or media unless
you have the right to do so; links and factual metadata do not transfer the
underlying work's copyright to FemRes.
...
提交贡献即表示你同意：代码与技术文档按 MIT 许可证发布；提交到
`src/content/` 的原创编辑文字与原创翻译按 CC BY-NC-SA 4.0 发布。请勿提交
无权再分发的第三方文字或媒体。
