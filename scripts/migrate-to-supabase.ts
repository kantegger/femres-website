 ...
 *
 * 使用方法：
 * 1. 确保 .env.local 或 .env 中同时有 NEON_DATABASE_URL (旧) 和 DATABASE_URL (新/Supabase)
 * 2. 运行: npm run migrate
 */

import postgres from 'postgres';
import { loadLocalEnv } from './load-env';
import { getDatabaseSslOptions } from '../src/lib/dbSsl';
...
if (!NEON_DATABASE_URL) {
  console.error('❌ 错误: .env.local 或 .env 中缺少 NEON_DATABASE_URL');
  console.log('请在 .env.local 或 .env 中添加:');
  console.log('NEON_DATABASE_URL=postgresql://neondb_owner:[PASSWORD]@ep-[...].neon.tech/neondb?sslmode=require');
  process.exit(1);
}

if (!DATABASE_URL) {
  console.error('❌ 错误: .env.local 或 .env 中缺少 DATABASE_URL');
  console.log('请在 .env.local 或 .env 中添加:');
  console.log('DATABASE_URL=postgresql://postgres:[PASSWORD]@db.[...].supabase.co:5432/postgres');
  process.exit(1);
}

const neonDatabaseUrl = NEON_DATABASE_URL;
const databaseUrl = DATABASE_URL;

// Neon 连接 (源数据库)
const neonSql = postgres(neonDatabaseUrl, {
  ssl: true,
  prepare: false,
  max: 1
});
...
// Supabase 连接 (目标数据库)
const supabaseSql = postgres(databaseUrl, {
...
async function main() {
  ...
  console.log('\n连接信息:');
  console.log(`  Neon: ${neonDatabaseUrl.replace(/:[^:]+@/, ':****@')}`);
  console.log(`  Supabase: ${databaseUrl.replace(/:[^:]+@/, ':****@')}`);
